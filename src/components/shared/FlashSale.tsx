...
const FlashSale = async () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-y-5 gap-x-1 my-40">
      {flashSaleProducts.slice(0, 10).map((product: TProduct) => (
        ...
        >
          ...
          <div className="card-body">
            <div className="badge badge-secondary text-[14px]">Flash Sale</div>
            <h2 className="card-title">{product.name}!</h2>

            <div className="card-actions ">
              <div className="badge badge-outline">Details</div>
              <div className="badge badge-outline">{product.price} $USD</div>
            </div>
          </div>
        </div>
      ))}
